#!/bin/bash
# Мониторинг во время apply-yaml-stage2: логи и метрики.
# При первом появлении ошибок записывает момент в alerts/first-error-stage2.txt

VL_URL="${VL_URL:-https://victorialogs.apatsev.org.ru}"
VM_URL="${VM_URL:-https://vmselect.apatsev.org.ru}"
RESULT_FILE="$(dirname "$0")/first-error-stage2.txt"
...
echo "Мониторинг этапа 2: VL=$VL_URL VM=$VM_URL интервал ${INTERVAL}s. Старт: $START_TS"
...
while true; do
  ...
  if has_errors; then
    ...
    {
      echo "# Первое появление ошибок во время apply-yaml-stage2"
