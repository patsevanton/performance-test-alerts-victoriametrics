...
| Метрика | Зачем смотреть | Порядок роста (прогон) |
| --- | --- | --- |
| `count(ALERTS)` / `count(ALERTS_FOR_STATE)` | Масштаб сценария и запись состояния | **~500** → **~50 000** ≈ **100×** |
...
| RPS и `apiserver_request_duration_seconds` (kube-apiserver) | Нагрузка на API при apply и reconcile | В таблице: RPS **~11** → **~14**, p99 **34 ms** → **до ~153 ms**, CPU **~70m** → **~110m** — рост умеренный, но мониторить вместе с data plane |
