...
Коротко по метрикам в таблице и как их интерпретировать:
...
- `vm_concurrent_select_limit_reached_total (increase[5m])` — сколько раз за 5 минут запросы упирались в лимит параллельности; ненулевые значения означают насыщение и риск ошибок/очередей. На том же участке показатель вырос с **106** до **5347**, а через ~1 час на том же уровне `ALERTS` вернулся к **0**.
- `vmselect_request_duration_seconds` (p95) — 95-й перцентиль длительности запросов к `vmselect`; это ранний индикатор деградации «стоимости» запроса. При росте нагрузки с ~45000 до ~50000 `ALERTS` RPS `vmselect` вырос с **7.6** до **8.6 req/s** (около **+13%**), а p95 — с **32.5** до **57.1 ms** (около **+76%**); затем p95 восстановился до **6.4 ms**.
- Даже в пике абсолютная latency оставалась низкой: максимум p95 составил **57.1 ms** (0.057 s), что примерно в **~1050x** ниже `search.maxQueueDuration=60s` и в **~5250x** ниже `search.maxQueryDuration=300s`. При устойчивом росте `current`/`limit_reached` имеет смысл повышать `search.maxConcurrentRequests` и `search.maxQueueDuration`.
