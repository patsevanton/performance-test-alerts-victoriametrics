...
- **Потребление ресурсов и масштабируемость:** На финальном срезе (~~50k `ALERTS`, см. таблицы) vmalert — порядка **~0,18** ядра CPU и **~1,43 Gi** RAM на реплику в среднем по pod, в пределах лимитов (4 CPU / 4 Gi в `vmks-values.yaml`). В пике `max(vmalert_iteration_duration_seconds)` доходила до **~6,4 с**, с запасом относительно `interval` группы **30 s**; для эксплуатации в диапазоне до **~50k** алертов достаточно заранее держать план по шардированию vmalert и масштабированию VMCluster на случай роста нагрузки. **ConfigMap'ы:** Operator стабильно дробит правила у лимита ~~1 MiB; при ~50k алертов фиксировалось 39 ConfigMap'ов (~~19,2 MB суммарно) — механизм предсказуем.
...
2. **Потребление ресурсов:** На **~50k** алертов запас по CPU vmalert относительно лимита 4 CPU остаётся существенным (на срезе около **~0,18 CPU** на реплику); для целевого диапазона до **~50k** алертов имеет смысл заранее держать план шардирования vmalert и масштабирования VMCluster. vmstorage на срезах — до **~2,6 Gi** RAM на реплику по таблице; это нужно закладывать в планирование нод.
