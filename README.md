...
| `vmalert_iteration_duration_seconds`               | **Риск:** если длительность итерации близка или больше `interval`, часть итераций пропускается, алерты срабатывают поздно, короткие всплески можно пропустить. **Что делать:** увеличить `interval`, вынести тяжёлые правила в отдельные группы/шарды, ускорить backend (vmselect/VMCluster), уменьшить сложность запросов. | До **~6,4 с**                 |
...
- **Потребление ресурсов и масштабируемость:** На финальном срезе (~~50k `ALERTS`, см. таблицы) vmalert — порядка **~0,18** ядра CPU и **~1,43 Gi** RAM на реплику в среднем по pod, в пределах лимитов (4 CPU / 4 Gi в `vmks-values.yaml`). В пике `max(vmalert_iteration_duration_seconds)` доходила до **~6,4 с**, с запасом относительно `interval` группы **30 s**; дальнейший рост к **~100k** алертов в эксплуатации разумно планировать с шардированием vmalert и масштабированием VMCluster. **ConfigMap'ы:** Operator стабильно дробит правила у лимита ~~1 MiB; при ~50k алертов фиксировалось 39 ConfigMap'ов (~~19,2 MB суммарно) — механизм предсказуем.
- **Нагрузка на API server:** По срезам Capacity Planning при росте `count(ALERTS)` с ~~500 до ~50 000 RPS API server оставался умеренным (**~11,9–13,9** req/s в среднем по окнам), p99 задержки **42–92 ms**, CPU kube-apiserver **~76–101m** — нагрузка на control plane заметна, но в этом прогоне не выступала главным узким местом по сравнению с data plane.
...
2. **Потребление ресурсов:** На **~50k** алертов запас по CPU vmalert относительно лимита 4 CPU остаётся существенным (на срезе около **~0,18 CPU** на реплику); при целевых **~100k** алертов всё равно имеет смысл планировать шардирование vmalert и масштабирование VMCluster заранее. vmstorage на срезах — до **~2,6 Gi** RAM на реплику по таблице; это нужно закладывать в планирование нод.
