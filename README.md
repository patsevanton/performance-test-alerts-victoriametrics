...
- оценить устойчивость и ресурсоёмкость стека: длительность и стабильность цикла оценки правил в `vmalert` (время итерации, отставание от `evaluation_interval`); ошибки и сбои выполнения `expr` (логи, метрики `vmalert`); динамику числа временных рядов в экспортируемых метриках `ALERTS` и `ALERTS_FOR_STATE` и влияние на TSDB (рост активных рядов, память, задержки запросов); признаки деградации при массовом применении правил (рост задержек, OOM, рестарты, потеря «памяти» состояний).
