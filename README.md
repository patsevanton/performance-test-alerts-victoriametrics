...
| --- | --- | --- |
| `container_cpu_usage_seconds_total` (pod vmalert) | Рост CPU стал заметно круче; при увеличении **~52 m** CPU на 1000 `ALERTS` нужен запас по лимитам и наблюдение за пиками | **~52 m** CPU на 1000 `ALERTS` |
| `container_memory_working_set_bytes` (pod vmalert) | Возможен OOM, при увеличении **~28 MiB** на 1000 `ALERTS` нужно закладывать память с запасом | **~28 MiB** на 1000 `ALERTS` |
| `vmalert_iteration_duration_seconds` | В пике доходило до **~31,1 с** (около `interval=30s`), возможны пропуски итераций при пиковых окнах | До **~31,1 с** |

### vmselect и запросы к TSDB

| Метрика | Чем грозит рост метрики | Порядок роста (прогон) |
| --- | --- | --- |
| `vm_http_requests_total{job="vmselect"}` | Перегрузка чтения TSDB и CPU vmselect, рост латентности и очередей при eval, таймауты запросов от vmalert и других клиентов. | **~39** req/s на 1000 `ALERTS` |
...
| Метрика | Чем грозит рост метрики | Порядок роста (прогон) |
| --- | --- | --- |
| `container_memory_working_set_bytes` (pod vmstorage) | Рост памяти заметный; при увеличении **~60 MiB** на 1000 `ALERTS` нужен пересмотр memory budget нод | **~60 MiB** на 1000 `ALERTS` |
| `container_cpu_usage_seconds_total` (pod vmstorage) | Тротлинг менее вероятен, но при росте **~5,3 m** CPU на 1000 `ALERTS` нужен контроль пиковых окон | **~5,3 m** CPU на 1000 `ALERTS` |
...
| Метрика | Чем грозит рост метрики | Порядок роста (прогон) |
| --- | --- | --- |
| `process_cpu_seconds_total` / CPU пода operator | Задержки reconcile и отставание применения конфигурации при перегрузке CPU | **~2,4 m** CPU на 1000 `ALERTS` |
