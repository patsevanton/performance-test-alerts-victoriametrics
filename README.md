...
| Момент / этап          | VMRule (всего) | Активные алерты (ALERTS) | ConfigMap'ов | CPU vmalert (реплика) | Memory vmalert (реплика) | CPU Operator | API Server - HTTP Requests by code | Примечание                                                                 |
| ---------------------- | -------------- | ------------------------ | ------------ | --------------------- | ------------------------ | ------------ | ---------------------------------- | -------------------------------------------------------------------------- |
...
| Снимок ~42 000 алертов | **389**        | **~42 214**              | **28**       | **~1250–1464m**       | **~867–1020Mi**          | **96m**      | **~13.1 RPS**                      | API Server из dashboard `Kubernetes / System / API Server`; `apiserver` RPS ~13.1, p99 ~60s |
