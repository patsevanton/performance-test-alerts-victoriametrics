...
- **Запас по циклу eval (периодический пересчёт/оценка всех правил в `vmalert`) был достаточным:** пик `max(vmalert_iteration_duration_seconds)` достигал **~6,4 с** при `interval` **30 s**.
- **Линейные ориентиры по vmalert:** В прогоне рост `ALERTS` давал примерно **~5,2 m CPU** и **~14,1 MiB RAM** на каждые 1000 `ALERTS`.
- **Линейные ориентиры по vmstorage:** В этом прогоне на каждые 1000 `ALERTS` приходилось примерно **~44,4 MiB RAM** (`container_memory_working_set_bytes`) и **~5,3 m CPU** (`container_cpu_usage_seconds_total`).
- **Линейные ориентиры по operator:** В прогоне на каждые 1000 `ALERTS` приходилось примерно **~1,6 m CPU** (`process_cpu_seconds_total`) и **~3,5 MiB RAM** (`process_resident_memory_bytes`).
