...
| Момент / этап          | VMRule (всего) | Активные алерты (ALERTS) | ConfigMap'ов | CPU vmalert (реплика) | Memory vmalert (реплика) | CPU Operator | API Server из dashboard `Kubernetes / System / API Server` | Примечание                                                                 |
| ---------------------- | -------------- | ------------------------ | ------------ | --------------------- | ------------------------ | ------------ | ---------------------------------- | -------------------------------------------------------------------------- |
...
| Метрика                  | ~23 000 алертов (факт) | ~84 000 алертов | ~100 000 алертов |
| ------------------------ | ---------------------- | --------------- | ---------------- |
| VMRule                   | ~252                   | ~920            | ~1 100           |
| CPU vmalert (реплика)    | ~723m                  | ~2.6 CPU        | ~3.1 CPU         |
| Memory vmalert (реплика) | ~676Mi                 | ~2.5Gi          | ~2.9Gi           |
| ConfigMap'ов             | 17                     | ~62             | ~74              |
| Временные ряды           | 4.79M                  | ~17.5M          | ~20.8M           |


Вывод: при линейной экстраполяции до **~100 000 алертов** ожидается рост потребления `vmalert` до ~3.1 CPU и ~2.9 Gi RAM на реплику. Уже в районе ~84 000 алертов CPU приближается к верхней границе типичного лимита 2–4 CPU. `max(vmalert_iteration_duration_seconds)` при ~23 000 алертов уже достигает 3.84 сек — при дальнейшем росте итерация может не укладываться в `interval`, что приведёт к росту `vmalert_iteration_missed_total`. Также потребуется масштабирование VMCluster (vmselect/vmstorage) из-за роста числа временных рядов.
