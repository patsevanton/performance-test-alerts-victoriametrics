...
Исследовать поведение VictoriaMetrics stack при нагрузке большим количеством алертов (VMRule) в Kubernetes-окружении. В частности:
...
- определить практические пороги масштабируемости и дать рекомендации по эксплуатации при большом числе правил оповещений.
...
ALERTS_FOR_STATE содержит полную информацию о состоянии алертов, необходимую для восстановления после рестарта vmalert. При запуске vmalert читает только этот ряд через -remoteRead.url для однократного восстановления состояния именно для настроенных правил.​

ALERTS полезен для мониторинга и анализа трендов алертов исторически, в то время как ALERTS_FOR_STATE служит исключительно для персистентности состояния памяти и не обновляется при хот-рилоаде конфигурации. Оба ряда пишутся через -remoteWrite.url в формате протокола remote-write.​
