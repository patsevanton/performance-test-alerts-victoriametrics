...
Суммарный размер `rulefiles-`*: **7 980 440 bytes (~7.61 MiB)**, средний размер ConfigMap: **~499 KB**.
Operator упаковывает правила в ConfigMap'ы до ~499–521 KB и создаёт новый по мере роста числа `VMRule`. На текущий момент 16 ConfigMap'ов (rulefiles-0 … rulefiles-15), последний заполнен частично.

### Перезапуски vmalert и восстановление состояния

`vmalert` остаётся в ожидаемой модели: при появлении нового ConfigMap возможен rolling restart (из-за новых `volume`/`volumeMount`), а восстановление состояния идёт через `remoteRead` по `ALERTS_FOR_STATE`. За время apply-yaml.sh зафиксировано 11 ReplicaSet'ов (10 пересозданий). При 196 тестовых VMRule и 16 ConfigMap'ов система стабильна: `vmalert_execution_errors_total = 0`, `vmalert_iteration_missed_total = 0`.
...
> - **Этап (алерты)** — приблизительное количество **сконфигурированных правил** (alert rules) на момент снятия снимка (число VMRule × 100 алертов в каждом).
...
- Отслеживать рост CPU vmalert при продолжении теста (при ~20 000 алертов — ~642m, запас до лимита 4 CPU значительный)
- Мониторинг `vmalert_iteration_missed_total` и `vmalert_iteration_duration_seconds` (текущий max 3.46 сек — далеко от `interval` 30s, но рост с 0.87 сек при ~5 000 алертов заметен)
- Контролировать память vmstorage (~2 201 Mi на реплику при ~20 000 алертов; лимит 4 Gi — осталось ~45% запаса)
- Размер `/metrics` vmalert уже ~24.6 MiB — отслеживать приближение к `maxScrapeSize` (128 MB)
