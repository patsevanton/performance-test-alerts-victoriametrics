...
Коротко по метрикам в таблице:

- `vm_concurrent_select_current` — текущее число одновременно выполняемых select-запросов в `vmselect`; рост показывает, что параллелизм занят сильнее и что можно увеличить `search.maxConcurrentRequests`.
- `vm_concurrent_select_limit_reached_total (increase[5m])` — сколько раз за последние 5 минут запросы упирались в лимит параллельности; ненулевые значения означают насыщение и риск ошибок/очередей. В этом случае можно увеличить `search.maxConcurrentRequests` и `search.maxQueueDuration`.
- `vmselect_request_duration_seconds` (p95) — 95-й перцентиль длительности запросов к `vmselect`; в этом прогоне p95 растет, но абсолютные значения остаются низкими (десятки миллисекунд), поэтому это скорее ранний индикатор тренда, чем признак критичной деградации latency.

На что обращать внимание:

- В реальном прогоне p95 оставался стабильно ниже `search.maxQueueDuration=60s` и далеко от `search.maxQueryDuration=300s`: максимум p95 составил **57.1 ms** (0.057 s), то есть примерно в **~1050x** ниже `maxQueueDuration` и в **~5250x** ниже `maxQueryDuration`.
- В динамике с RPS видно ухудшение «стоимости» запросов на верхней ступени: при росте нагрузки с ~45000 до ~50000 `ALERTS` RPS `vmselect` вырос с **7.6** до **8.6 req/s** (около **+13%**), а p95 — с **32.5** до **57.1 ms** (около **+76%**).
- На насыщении vmselect это подтверждается одновременно тремя метриками: `vm_concurrent_select_current` вырос с **0** до **16**, `vm_concurrent_select_limit_reached_total (increase[5m])` — с **106** до **5347**, а p95 — с **32.5** до **57.1 ms** (участок ~45000 -> ~50000 `ALERTS`).
- По длительности деградации пик не был постоянным: повышенный p95 держался минимум на двух соседних шагах (~45000 и ~50000 `ALERTS`), после чего через ~1 час на том же уровне `ALERTS` p95 восстановился до **6.4 ms**, а `limit_reached` вернулся к **0**.
