...
> **Снимки Capacity Planning:** чтобы собрать замеры загрузки при ~500, 5000, … 50000 активных `ALERTS`, запустите **до начала** `deploy-apps.sh` в отдельном терминале скрипт `python3 scripts/fetch_capacity_snapshots.py` — он опрашивает `count(ALERTS)` каждые `POLL_INTERVAL` секунд и фиксирует снимок при достижении очередного порога. Если запустить его после деплоя, первые пороги будут пропущены. Подробности — в разделе [Capacity Planning](#capacity-planning).
