...
| Метрика                                                   | Чем грозит рост метрики и что делать                                                                                                                                                                                                                                            | Порядок роста (прогон)                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `container_memory_working_set_bytes` (pod vmstorage)      | Рост снижает memory headroom на ноде; держать запас и заранее масштабировать `vmstorage`.                                                                                                                                                                                      | **~44,4 MiB** на 1000 `ALERTS`              |
| `container_cpu_usage_seconds_total` (pod vmstorage)       | Пики CPU повышают latency чтения/записи; контролировать лимиты и равномерность нагрузки.                                                                                                                                                                                       | **~5,3 m** CPU на 1000 `ALERTS`             |
...
- **Ресурсы и масштабируемость предсказуемы:** На финальном срезе (~50k `ALERTS`) vmalert потреблял около **~0,18 CPU** и **~1,43 Gi RAM** на реплику при лимитах 4 CPU / 4 Gi; пик `max(vmalert_iteration_duration_seconds)` достигал **~6,4 с** при `interval` **30 s**.
- **Запас по циклу eval был достаточным:** `vmalert_iteration_duration_seconds` оставался заметно ниже `interval` (пик ~6,4 с при 30 с, запас ~4,7x); если метрика приближается к `interval`, возможны задержки срабатывания и пропуск коротких всплесков, поэтому нужно увеличивать `interval`, упрощать тяжёлые правила и/или масштабировать backend (`vmselect`/VMCluster).
...
- **Линейные ориентиры по vmstorage:** В этом прогоне на каждые 1000 `ALERTS` приходилось примерно **~44,4 MiB RAM** (`container_memory_working_set_bytes`) и **~5,3 m CPU** (`container_cpu_usage_seconds_total`); эти значения использовать как baseline для запаса и раннего масштабирования.
...
- **Нагрузка на vmselect:** ориентир — около **~6,8 req/s** на каждую 1000 `ALERTS`; при дальнейшем росте нужны ранний мониторинг таймаутов и масштабирование `vmselect` (при необходимости — `vmstorage`).
