...
| -------------- | -------- |
| Всего правил | 67 500 |
| Используют `(real_query) or vector(fallback)` | 61 247 (90,7%) |
| `absent(...)` без обращения к TSDB | 6 253 (9,3%) |
| `histogram_quantile` (внутри `or vector()`) | 4 234 |
| `rate()` (внутри `or vector()`) | 28 547 |
| `avg_over_time` (внутри `or vector()`) | 2 329 |
| Subqueries (`[5m:1m]` и подобные) | 0 |
| Joins (`group_left` / `group_right`) | 0 |
| `label_join` / `label_replace` | 0 |

Все 67 500 правил построены по схеме `(real_metric_expression) or vector(N)`. Поскольку на тестовом стенде реальные прикладные метрики (например, `http_request_duration_seconds_bucket`, `kube_pod_container_status_restarts_total` от тестовых сервисов) **не существуют**, реальная часть выражения возвращает пустой результат, и вычисление сводится к `vector(N) > threshold` — практически бесплатная операция для query engine.

**Что это означает для интерпретации результатов:**

- Измеренное потребление CPU и RAM компонентами `vmselect` и `vmstorage` отражает главным образом **overhead управления 67 500 алертами** (запись/чтение рядов `ALERTS` и `ALERTS_FOR_STATE`), а **не** стоимость выполнения тяжёлых запросов к реальным данным.
...
- Тест показывает **нижнюю границу** потребления ресурсов — «стоимость самого факта существования 67 500 алертов» без учёта стоимости вычисления сложных выражений.
