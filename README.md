...
- `vm_concurrent_select_current` — текущее число одновременно выполняемых select-запросов в `vmselect`; рост показывает, что параллелизм занят сильнее.
- `vm_concurrent_select_limit_reached_total (increase[5m])` — сколько раз за последние 5 минут запросы упирались в лимит параллельности; ненулевые значения означают насыщение и риск ошибок/очередей.
- `vmselect_request_duration_seconds` (p95) — 95-й перцентиль длительности запросов к `vmselect`; рост p95 указывает на деградацию latency и замедление eval в `vmalert`.
...
Для удобства чтения ориентир такой: сначала смотрят `vm_concurrent_select_current` (насколько заполнен параллелизм), затем `vm_concurrent_select_limit_reached_total` (начались ли отказы по лимиту), и вместе с ними p95 `vmselect_request_duration_seconds` (как это влияет на задержку выполнения запросов).
