| ~25000 | 13.3           | 50 ms              | 87m            | 994.8             | 0.1                | 11.7              |
| ~30000 | 12.8           | 39 ms              | 84m            | 1146.2            | 0.1                | 11.8              |
| ~35000 | 12.4           | 48 ms              | 86m            | 1134.8            | 0.1                | 11.8              |
| ~40000 | 13.0           | 41 ms              | 87m            | 1389.8            | 0.1                | 12.2              |
| ~45000 | 13.3           | 96 ms              | 96m            | 1749              | 0.1                | 12.7              |
| ~50000 | 13.6           | 149 ms             | 98m            | 1952.9            | 0.1                | 12.6              |
...
| --- | --- | --- |
| `container_cpu_usage_seconds_total` (pod vmalert) | **Риск:** vmalert начинает "задыхаться", правила считаются дольше, растут задержки алертов. **Что делать:** поднять `requests/limits` CPU, добавить реплики/шарды vmalert, увеличить `interval` у самых тяжёлых групп правил, упростить дорогие выражения. | **~52 m** CPU на 1000 `ALERTS` |
| `container_memory_working_set_bytes` (pod vmalert) | **Риск:** при нехватке памяти pod уходит в OOMKill и перезапускается, в этот момент возможна пауза в оценке правил. **Что делать:** увеличить memory limit с запасом, снизить кардинальность/число правил на инстанс, шардировать vmalert и следить за трендом памяти заранее. | **~28 MiB** на 1000 `ALERTS` |
| `vmalert_iteration_duration_seconds` | **Риск:** если длительность итерации близка или больше `interval`, часть итераций пропускается, алерты срабатывают поздно, короткие всплески можно пропустить. **Что делать:** увеличить `interval`, вынести тяжёлые правила в отдельные группы/шарды, ускорить backend (vmselect/VMCluster), уменьшить сложность запросов. | До **~31,1 с** |

### vmselect и запросы к TSDB

| Метрика | Чем грозит рост метрики и что делать | Порядок роста (прогон) |
| --- | --- | --- |
| `vm_http_requests_total{job="vmselect"}` | **Риск:** vmselect перегружается по CPU/диску, ответы становятся медленнее, vmalert и Grafana начинают получать таймауты. **Что делать:** масштабировать vmselect (и при необходимости vmstorage), включить шардирование vmalert, уменьшать частоту и "тяжесть" запросов в правилах. | **~39** req/s на 1000 `ALERTS` |
...
| Метрика | Чем грозит рост метрики и что делать | Порядок роста (прогон) |
| --- | --- | --- |
| `process_cpu_seconds_total` / CPU пода operator | **Риск:** reconcile начинает отставать, изменения VMRule применяются медленнее, дольше идёт схождение состояния. **Что делать:** увеличить CPU лимит operator, уменьшить "бурст" изменений (батчи/GitOps-волны), следить за длительностью reconcile и очередью событий. | **~2,4 m** CPU на 1000 `ALERTS` |
| `process_resident_memory_bytes` / память пода | **Риск:** при очень больших наборах правил возможен OOM и рестарт operator, что задержит обновления конфигурации. **Что делать:** добавить memory запас, контролировать количество/размер `VMRule`, периодически проверять число и размер `rulefiles` ConfigMap. | **~5,4 MiB** на 1000 `ALERTS` |
...
- **Отказоустойчивость:** Механизм remoteWrite/remoteRead (`ALERTS`, `ALERTS_FOR_STATE`) подтверждён на практике: после рестарта vmalert восстанавливает состояние алертов из VictoriaMetrics, счётчики `for` не сбрасываются, потери алертов не наблюдались. На отчётном прогоне `increase(vmalert_execution_errors_total[8h]) = 0`, а `increase(vmalert_iteration_missed_total[8h]) = 3` (единичные пропуски на пиковых окнах). Временное падение `sum(ALERTS)` во время рестарта — следствие задержки первой итерации, а не потери данных. **Перезапуски vmalert** при поэтапном применении правил: каждое появление нового ConfigMap приводило к пересозданию Pod'а (volume/volumeMount); интервал между рестартами ~9–11 мин, за период массового apply — 11 ReplicaSet'ов (10 пересозданий). Горячая перезагрузка (SIGHUP) возможна при обновлении существующих ConfigMap'ов без добавления новых.
- **Потребление ресурсов и масштабируемость:** На финальном срезе (~50k `ALERTS`, см. таблицы) vmalert — порядка **~2,6** ядра CPU и **~1,4 Gi** RAM на реплику в среднем по поду, в пределах лимитов (4 CPU / 4 Gi в `vmks-values.yaml`). В пике `max(vmalert_iteration_duration_seconds)` доходила до **~31,1 с**, что близко к `interval` группы **30 s**; на прогоне зафиксированы единичные пропуски (`increase(vmalert_iteration_missed_total[8h]) = 3`). Дальнейший рост к **~100k** алертов в эксплуатации разумно планировать с шардированием vmalert и масштабированием VMCluster. **ConfigMap'ы:** Operator стабильно дробит правила у лимита ~1 MiB; при ~50k алертов фиксировалось 39 ConfigMap'ов (~19,2 MB суммарно) — механизм предсказуем.
- **Нагрузка на API server:** По срезам Capacity Planning при росте `count(ALERTS)` с ~500 до ~50 000 RPS API server оставался умеренным (**~11,6–13,6** req/s в среднем по окнам), p99 задержки **30–149 ms**, CPU kube-apiserver **~73–98m** — нагрузка на control plane заметна, но в этом прогоне не выступала главным узким местом по сравнению с vmalert/vmselect/vmstorage.

### Основные выводы

1. **Отказоустойчивость:** Конфигурация с двумя репликами vmalert, remoteRead/remoteWrite и Alertmanager в кластерном режиме после полного развёртывания правил показала восстановление без потери состояния алертов и без дублирования уведомлений. RTO vmalert — в пределах минуты (детали — в комментариях к `vmks-values.yaml`). Итоговые показатели: `increase(vmalert_execution_errors_total[8h]) = 0`, `increase(vmalert_iteration_missed_total[8h]) = 3`. Для **следующих** массовых изменений набора VMRule стоит по-прежнему закладывать периодические рестарты vmalert (по одному на каждый новый ConfigMap) и при необходимости использовать батчи или GitOps с темпом — чтобы не создавать лишние ConfigMap'ы подряд.
2. **Потребление ресурсов:** На **~50k** алертов запас по CPU vmalert относительно лимита 4 CPU ещё есть, но пик уже доходил до **~2,6 CPU** на реплику; при целевых **~100k** алертов — ориентир на лимиты 3–4 CPU на реплику, шардирование vmalert и масштабирование VMCluster. vmstorage на пике — порядка **~3,2 Gi** RAM на реплику по таблице; это нужно закладывать в планирование нод.
