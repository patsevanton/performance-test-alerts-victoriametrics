...
**Задачи исследования** (в том же порядке):

- исследовать механизм сохранения и восстановления состояния алертов через `remoteWrite`/`remoteRead` и влияние рестартов на «память» алертов; при каких условиях пересоздаётся Pod `vmalert`;
- оценить увеличение потребления ресурсов при росте числа алертов; исследовать распределение правил VictoriaMetrics Operator по ConfigMap'ам при лимите ~1 MiB и рост числа объектов при добавлении `VMRule`;
- оценить рост нагрузки на `kube-apiserver` (RPS, задержки, CPU) в ходе прогона;
