...
| Метрика                                  | Значение                                    |
| ---------------------------------------- | ------------------------------------------- |
| VMRule в кластере (всего)                | **186** (147 тестовых + 39 встроенных)       |
| VMRule в namespace `vmks`                | **39**                                      |
| VMRule в namespace `default` (тестовые)  | **147**                                     |
| Целевое количество алертов               | **~50 000** (`500` VMRule × `100` алертов)   |
| Активные алерты (ALERTS)                 | **17 888**                                  |
| ALERTS_FOR_STATE                         | **14 688**                                  |
| sum(vmalert_alerts_firing)               | **19 748**                                  |
| Временные ряды (totalSeries)             | **2 549 698**                               |
| ConfigMap'ов с правилами (`rulefiles-`*) | **12**                                      |
| Суммарный размер ConfigMap'ов            | **6 020 483 bytes (~5.74 MiB)**             |
| ReplicaSet'ов vmalert                    | **11**                                      |
| sum(vmalert_execution_errors_total)      | **0**                                       |
| sum(vmalert_iteration_missed_total)      | **0**                                       |
| max(vmalert_iteration_duration_seconds)  | **2.68 сек**                                |


### Распределение правил по ConfigMap'ам

Суммарный размер `rulefiles-`*: **6 020 483 bytes (~5.74 MiB)**, средний размер ConfigMap: **~502 KB**.
Operator упаковывает правила в ConfigMap'ы до ~505–521 KB и создаёт новый по мере роста числа `VMRule`. На текущий момент 12 ConfigMap'ов (rulefiles-0 … rulefiles-11), последний заполнен частично.

### Перезапуски vmalert и восстановление состояния

`vmalert` остаётся в ожидаемой модели: при появлении нового ConfigMap возможен rolling restart (из-за новых `volume`/`volumeMount`), а восстановление состояния идёт через `remoteRead` по `ALERTS_FOR_STATE`. За время apply-yaml.sh зафиксировано 11 ReplicaSet'ов (10 пересозданий).
...
- Отслеживать рост CPU vmalert при продолжении теста (при ~14 700 алертов — ~510m, запас до лимита 4 CPU значительный)
- Мониторинг `vmalert_iteration_missed_total` и `vmalert_iteration_duration_seconds` (текущий max 2.68 сек — далеко от `interval` 30s, но рост с 0.87 сек при ~5 000 алертов заметен)
- Контролировать память vmstorage (~1 617 Mi на реплику при ~14 700 алертов; лимит 4 Gi)
- Размер `/metrics` vmalert уже ~18.7 MiB — отслеживать приближение к `maxScrapeSize` (128 MB)
