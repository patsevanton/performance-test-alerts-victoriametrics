...
> **Терминология в таблицах:**
> - **VMRule** — количество объектов `VMRule` в кластере на момент снятия снимка. Каждый `VMRule` содержит **~100 алертов**, поэтому общее число правил ≈ VMRule × 100.
> - **ALERTS** — количество **time series** `ALERTS` в TSDB (`count(ALERTS)`). Эта метрика содержит label `alertstate` (`"firing"` / `"pending"`), поэтому одно правило может порождать **два** ряда одновременно. Кроме того, две реплики vmalert оценивают правила независимо и могут записать разные `alertstate` для одного алерта в один момент. Из-за этого `count(ALERTS)` всегда **больше** числа правил.
> - **ALERTS_FOR_STATE** — количество time series `ALERTS_FOR_STATE` (один ряд на алерт, без разбивки по `alertstate`), поэтому это значение ближе к числу правил.

#### CPU (на реплику)


| VMRule | vmalert | vmstorage | vmselect | vminsert | vmagent | operator |
| ------ | ------- | --------- | -------- | -------- | ------- | -------- |
| ~50    | 139m    | 93m       | 87m      | 18m      | 50m     | 20m      |
| ~100   | 281m    | 152m      | 161m     | 22m      | 79m     | 50m      |
| ~147   | 510m    | 128m      | 212m     | 32m      | 125m    | 62m      |
| ~200   | 642m    | 779m      | 363m     | 37m      | 151m    | 92m      |
| ~250   | 1 102m  | 1 074m    | 547m     | 49m      | 213m    | 123m     |
| ~440   | 1 978m  | 302m      | 439m     | 83m      | 348m    | 180m     |
...
| VMRule | vmalert | vmstorage | vmselect | vminsert | vmagent | operator |
| ------ | ------- | --------- | -------- | -------- | ------- | -------- |
| ~50    | 121Mi   | 652Mi     | 112Mi    | 99Mi     | 85Mi    | 66Mi     |
| ~100   | 265Mi   | 1 329Mi   | 146Mi    | 108Mi    | 141Mi   | 97Mi     |
| ~147   | 385Mi   | 1 617Mi   | 295Mi    | 158Mi    | 158Mi   | 125Mi    |
| ~200   | 514Mi   | 2 201Mi   | 218Mi    | 141Mi    | 184Mi   | 157Mi    |
| ~250   | 625Mi   | 2 949Mi   | 1 071Mi  | 188Mi    | 360Mi   | 185Mi    |
| ~440   | 1 386Mi | 2 619Mi   | 305Mi    | 253Mi    | 320Mi   | 270Mi    |


### RPS и операционные метрики


| VMRule | API Server RPS | API Server p99 lat | API Server CPU | vmselect HTTP RPS | vmstorage HTTP RPS | vminsert HTTP RPS | vmalert iter_duration (max) | vmalert exec_errors | vmalert iter_missed | vmalert remotewrite_req |
| ------ | -------------- | ------------------ | -------------- | ----------------- | ------------------ | ----------------- | --------------------------- | ------------------- | ------------------- | ----------------------- |
| ~50    | 12.3           | 48 ms              | 81m            | 201               | 0.1                | 10.4              | 0.87 сек                    | 0                   | 0                   | 860                     |
| ~100   | 12.6           | 49 ms              | 81m            | 460               | 0.1                | 10.8              | 1.48 сек                    | 0                   | 0                   | 1 309                   |
| ~147   | 12.7           | 46 ms              | 88m            | 691               | 0.1                | 11.1              | 2.68 сек                    | 0                   | 0                   | 1 319                   |
| ~200   | 12.7           | 57 ms              | 95m            | 890               | 0.1                | 11.1              | 3.46 сек                    | 0                   | 0                   | 1 710                   |
| ~250   | 12.0           | 56 ms              | 93m            | 1 153             | 0.1                | 11.6              | 13.19 сек                   | 0                   | 0                   | 2 220                   |
| ~440   | 13.7           | 60 ms              | 101m           | 1 866             | 0.1                | 12.6              | 13.78 сек                   | 21                  | 0                   | 1 032 805               |


### Объёмные метрики и состояние


| VMRule | ALERTS  | ALERTS_FOR_STATE | ConfigMaps (кол-во) | ConfigMaps (размер) | scrape_body_size (vmalert) |
| ------ | ------- | ---------------- | ------------------- | ------------------- | -------------------------- |
| ~50    | 5 116   | 4 153            | 4                   | 1.74 MiB            | 5.3 MiB                    |
| ~100   | 11 340  | 9 407            | 8                   | 3.76 MiB            | 12.0 MiB                   |
| ~147   | 17 888  | 14 688           | 12                  | 5.74 MiB            | 18.7 MiB                   |
| ~200   | 28 142  | 19 511           | 16                  | 7.61 MiB            | 24.6 MiB                   |
| ~250   | 32 685  | 24 728           | 20                  | 9.59 MiB            | 31.6 MiB                   |
| ~440   | 45 446  | 40 955           | 32                  | 15.85 MiB           | 50.6 MiB                   |
